...
import ApiObjects.CurrentlyPlayingResponse
...
play accessToken = do
  request <-
    parseRequest "https://api.spotify.com/v1/me/player/play"
      <&> method .~ "PUT"
...
pause accessToken = do
  request <-
    parseRequest "https://api.spotify.com/v1/me/player/pause"
      <&> method .~ "PUT"
      <&> requestHeaders .~ [toAuthorizationHeader accessToken]
  manager <- newManager tlsManagerSettings
  response <- httpLbs request manager
...
next accessToken = do
  request <-
    parseRequest "https://api.spotify.com/v1/me/player/next"
      <&> method .~ "POST"
      <&> requestHeaders .~ [toAuthorizationHeader accessToken]
  manager <- newManager tlsManagerSettings
  response <- httpLbs request manager
...
previous accessToken = do
  request <-
    parseRequest "https://api.spotify.com/v1/me/player/previous"
      <&> method .~ "POST"
      <&> requestHeaders .~ [toAuthorizationHeader accessToken]
  manager <- newManager tlsManagerSettings
  response <- httpLbs request manager
...
setVolume accessToken volumePercent = do
  request <-
    parseRequest "https://api.spotify.com/v1/me/player/volume"
      <&> method .~ "PUT"
      <&> requestHeaders .~ [toAuthorizationHeader accessToken]
...
setPlayer accessToken deviceId = do
  request <-
    parseRequest "https://api.spotify.com/v1/me/player"
      <&> method .~ "PUT"
      <&> requestHeaders .~ [toAuthorizationHeader accessToken]
...
getCurrentlyPlaying accessToken = do
  request <-
    parseRequest "https://api.spotify.com/v1/me/player/currently-playing"
      <&> method .~ "GET"
      <&> requestHeaders .~ [toAuthorizationHeader accessToken]
  manager <- newManager tlsManagerSettings
  response <- httpLbs request manager
...
getAvailableDevices accessToken = do
  request <-
    parseRequest "https://api.spotify.com/v1/me/player/devices"
      <&> method .~ "GET"
      <&> requestHeaders .~ [toAuthorizationHeader accessToken]
  manager <- newManager tlsManagerSettings
  response <- httpLbs request manager
...
searchTrack accessToken query = do
  request <-
    parseRequest "https://api.spotify.com/v1/search"
      <&> method .~ "GET"
      <&> requestHeaders .~ [toAuthorizationHeader accessToken]
      <&> queryString
        .~ [ ("q", Just query),
